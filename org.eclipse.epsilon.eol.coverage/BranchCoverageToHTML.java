...
/*
 * This class is unfinished, see the accompanying report
 * Creates a HTML output for branch coverage
 */
public class BranchCoverageToHTML {
	...
	public BranchCoverageToHTML(AST ast, File targetFile, File inputFile) {
		this.ast = ast;
	...
	public static void main(String[] args) {
		...
		BranchCoverageToHTML output = new BranchCoverageToHTML(ast,
	...
	private void dfAST(AST ast) {
		...
		if (ast.getType() != EolLexer.EOLMODULE && !ast.isImaginary()) {
			statementCount++;		
			if (ast.getCFG().getNumberOfBranches() > 1) {
				...
				for (int i = ast.getRegion().getStart().getColumn(); i <= ast.getRegion().getEnd().getColumn() && i < array.length; i++) {
					if (ast.getCFG().getNumberOfBranches() == ast.getCFG().getNumberOfExecutedBranches())
						array[i] = 'G';
					else if (ast.getCFG().getNumberOfExecutedBranches() > 0)
						array[i] = 'Y';
					else
						array[i] = 'R';
				}
	...
	private void outputCoverageStats(BufferedWriter writer) throws IOException {
		...
		float percentage = ((float)100 / (float)statementCount) * (float)coveredStatementCount;
		writer.write(Math.round(percentage) + "%<br><br>");
	}

	private void enableHighlighting(BufferedWriter writer, String colour) throws IOException {
		writer.write("<span style=\"background-color: #" + colour + "\">");
	}
	...
	private void outputEOLFile(BufferedWriter writer) throws IOException {
		...
		for (int i = 0; i < fileContents.size(); i++) {
			...
			for (int j = 0; j < line.length(); j++) {
				if (covered[i].charAt(j) != 'N' && !highlightingEnabled) {
					switch (covered[i].charAt(j)) {
					case 'R':
						this.enableHighlighting(writer, "FF0000");
						break;
					case 'Y':
						this.enableHighlighting(writer, "FFFF00");
						break;
					case 'G':
						this.enableHighlighting(writer, "5EDA57");
						break;
					}
