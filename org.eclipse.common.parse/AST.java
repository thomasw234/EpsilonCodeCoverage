...
public class AST extends CommonTree {
	...
	private boolean statementVisited = false;
	
	// Keeps track of executed blocks of a conditional statement
	private List<Integer> executedDescendents = null;
	
	private CFG cfg = null;
	
	
	private ControlFlowNode thisCFN;
	...
	public void setVisited(int i) {
		if (executedDescendents == null) {
			executedDescendents = new ArrayList<Integer>();
		}
		
		executedDescendents.add(i);
	}
	...
	public boolean getVisited(int i) {
		...
		return executedDescendents.contains(i);
	}
	...
	{
		this.setTree(tree);
		System.out.println("settree called");
	}
	...
	public int getID()
	{
		if (this.ID == -1)
		{
			this.assignIDs();
			if (this.ID == -1) {
				System.out.println("Assign ID's isn't working");
			}
		}
	...
	public void assignIDs() {
		...
		while (current.getType() != 60 && current.getParent() != null) current = current.getParent();
		
		this.recusiveIDassign(current, new MutableInt(0));
		
	}
	
	private void recusiveIDassign(AST ast, MutableInt next) {
		ast.ID = next.value;
		System.out.print(ast.ID);
		for (AST child : ast.getChildren()) {
			++next.value;
			this.recusiveIDassign(child, next);
		}
	}
	...
	public boolean getVisited() {
		return this.statementVisited;
	}
	
	public void setVisited() {
		this.statementVisited = true;
	}
