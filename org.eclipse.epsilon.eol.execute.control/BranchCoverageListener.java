...
public class BranchCoverageListener implements IExecutionListener {

	public BranchCoverageListener() {
	...
	public void aboutToExecute(AST ast, IEolContext context) {
		...
		// Check that we're not an ignored branch
		//if (ast.getParent() != null && ast.getParent().getType() == EolLexer.FOR && ast.getType() != EolLexer.BLOCK)
		//return;
			...
			if (last.getCFG().getChildCount() > 1) {
				...
				if (current.getParent() != null &&
					...
					if ((last.getType() == EolLexer.FOR || last.getType() == EolLexer.WHILE) &&
						...
						last.getCFG().markBranchExecuted(last.getCFG().getChild(1));
					}

					// Otherwise mark the branch as executed
					current.getParent().getCFG().markBranchExecuted(current.getCFG());
				}
					...
					for (CFG parent : current.getCFG().getAllParents()) {
						if (parent.equals(last.getCFG())) {
							parent.markBranchExecuted(current.getCFG());
							break;
						}
					}
				}
	...
	public void finishedExecuting(AST ast, Object result, IEolContext context) {
		...
		if ((ast.getType() == EolLexer.FOR || ast.getType() == EolLexer.WHILE)
			...
			last.getCFG().markBranchExecuted(ast.getCFG());
			if (AST2CFG.validType(ast.getCFG())) {
				last = ast;
			}
		}
	...
	private static boolean sameOperation(AST first, AST second) {
		if (first.getCFG().getUniqueID() == 5 && second.getCFG().getUniqueID() == 2)
			System.out.println();
	...
	public void markFinished(CFG end) {
		if (last != null)
			last.getCFG().markBranchExecuted(end);
	}
